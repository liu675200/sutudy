...
void UART0_IRQHandler(void)
{
    ...
    switch(UART0->OFFSET_8.IIR & 0x0f)
    {
        ...
        case ((uint32_t)0x0001)://未发生中断
                    {
                        __kfifo_put(&uart0RxFifo,1);
                    }
        break;

        case ((uint32_t)0x0002)://发送寄存器空中断
                    {
        ...
        case ((uint32_t)0x0000)://Modem状态中断
                    {
