...
void DMA_Config_M2M(void)//M2M模式，DAM1
{
	DMA_InitTypeDef DMA_InitStructure;

  //*add1 = (uint32_t)(sourth1);
  //*add2 = (uint32_t)(mudi1);

  // 开DMA时钟
  RCC_AHBENR |= ((uint32_t)0x00000001);
  // 源数据地址
  //DMA_InitStructure.DMA_PeripheralBaseAddr = (*add1);
  ...
  NVIC_Configuration(DMA1_Channel6_IRQn,1,1);
...
void DMA_Config_M2P(uint32_t ADD)//M2P模式，内存到串口发送寄存器,可以弄一个（1<<x）x用宏定义来快捷设置
{
  ...
  NVIC_Configuration(DMA1_Channel4_IRQn,1,1);//NVIC与优先级配置
  DMA1_Channel4->CCR |= (1<<1); //使能传输完成TC中断
  DMA1_Channel4->CCR |= ((uint16_t)0x0001);// 使能DMA线6
}
