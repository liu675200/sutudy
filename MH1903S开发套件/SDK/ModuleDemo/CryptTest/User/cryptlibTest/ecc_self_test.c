...
#if 0
extern uint32_t ecc_padd_a(ecc_point_a *r,ecc_point_a *a,ecc_point_a *b,const ecc_para *para);
extern uint32_t ecc_padd_ja(ecc_point_j *r,ecc_point_j *a,ecc_point_a *b,const ecc_para *para);
extern uint32_t ecc_pdbl_a(ecc_point_a *r,ecc_point_a *a,const ecc_para *para);
extern uint32_t ecc_pdbl_j(ecc_point_j *r,ecc_point_j *a,const ecc_para *para);
extern uint32_t ecc_pdbl_mj(ecc_point_mj *r,ecc_point_mj *a,const ecc_para *para);
...
extern uint32_t ecc_verifypoint(ecc_point_a *a,ecc_para *para);
extern void ecc_j2a(ecc_point_a *r,ecc_point_j *a,const ecc_para *para);
extern void ecc_j2mj(ecc_point_mj *r,ecc_point_j *a,const ecc_para *para);
extern void ecc_j2mj_t(uint32_t *r,uint32_t *z,const ecc_para *para);
...
typedef struct
{
	uint32_t p[8];				//模数--由外部传入
	uint32_t a[8];				//椭圆曲线常数a--由外部传入
	uint32_t b[8];				//椭圆曲线常数b--由外部传入
	ecc_point_a_value g;			//基点--由外部传入
	uint32_t n[8];				//基点的阶--由外部传入
	uint32_t p_c[8];			//以n为取模的常量c
	/*
	uint32_t p_q;				//以n为取模的调整因子q
	*/
	uint32_t n_c[8];			//以p为取模的常量c
	/*
	...
	*/
}ecc_para_value;				//ecc输入参数的数据结构

typedef struct
{
	uint32_t d[8];				//ecc私钥
	ecc_point_a_value e; 		//ecc公钥
}ecc_key_value;					//ecc密钥的数据结构
...
void ecc_pdbl_a_test(void);
void ecc_pdbl_mj_test(void);
void ecc_padd_a_test(void);
void ecc_padd_ja_test(void);
void ecc_pmul_calc_test(void);
...
{
	DBG_PRINT("x:\n");
	bn_printf(p->x, len_words);DBG_PRINT("\n");
	DBG_PRINT("y:\n");
	bn_printf(p->y, len_words);DBG_PRINT("\n");
}
...
void ecc_printf_para(ecc_para *p)
{
	...
	DBG_PRINT("len_bits:%d\n", p->len_bits);
	DBG_PRINT("len_words:%d\n", p->len_words);DBG_PRINT("\n");
	
	DBG_PRINT("p:\n");
	bn_printf(p->p, p->len_words);DBG_PRINT("\n");
	DBG_PRINT("a:\n");
	bn_printf(p->a, p->len_words);DBG_PRINT("\n");
	DBG_PRINT("b:\n");
	bn_printf(p->b, p->len_words);DBG_PRINT("\n");
	ecc_printf_point(&p->g, p->len_words);
	
	DBG_PRINT("n:\n");
	bn_printf(p->n, p->len_words);DBG_PRINT("\n");
	
	DBG_PRINT("p_C:\n");
	bn_printf(p->p_c, p->len_words);DBG_PRINT("\n");
	DBG_PRINT("p_Q:%X\n", p->p_q);
	
	DBG_PRINT("n_C:\n");
	bn_printf(p->n_c, p->len_words);DBG_PRINT("\n");
	DBG_PRINT("n_Q:%X\n", p->n_q);
...
// 	ecc_printf_point(&calc_point, para.len_words);
...
// 	ecc_printf_point(&exp_point, para.len_words);
	...
	ecc_pmul_calc(&calc_point, &(para.g), k, &para, mh_rand_p, NULL);
...
void ecc_naf_test(void)
{
	...
	if(0 == memcmp(exp_data, calc_data, sizeof(exp_data)))
		DBG_PRINT("ecc_naf generate data pass\n");
	else
		DBG_PRINT("ecc_naf generate data fail\n");
								
	if(exp_len == calc_len)
		DBG_PRINT("ecc_naf data len pass\n");
	else
		DBG_PRINT("ecc_naf data len fail\n");
	
	if(exp_oe == calc_oe)
		DBG_PRINT("ecc_naf data oe pass\n");
	else
		DBG_PRINT("ecc_naf data oe fail\n");
...
void ecc_naf_point_test()
{
	...
	if(0 == memcmp(calc_naf_v, exp_naf_v, sizeof(exp_naf_v)))
		DBG_PRINT("ecc_naf_point data pass\n");
	else
		DBG_PRINT("ecc_naf_point data fail\n");
