...
int main(void)
{
	...
	memset(&USB_OTG_Core_dev, 0x00, sizeof(USB_OTG_Core_dev));
    ...
    printf("MegaHunt SCPU HID Host Demo V1.0.\r\n");
	...
	while(1)
    {
        ...
        if ((USBID_status()))
        {
            USB_vbus_CMD(&USB_OTG_Core_dev, DISABLE);
        }
		else
		{
            USB_vbus_CMD(&USB_OTG_Core_dev, ENABLE);
        }           
    }
}
